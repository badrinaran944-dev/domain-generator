...
const navItems = ['Home', 'Collections', 'Offers', 'New Launch', 'Features'];
...
function App() {
  return (
    <main className="wallcool-app min-h-screen text-slate-100">
      ...
      <section className="container section">
        ...
        <div className="cards">
          {animeCards.map((card) => (
            <article key={card.title} className="anime-card">
              <div className="thumb" />
              <h4>{card.title}</h4>
              <p>{card.genre}</p>
              <div className="row-between">
                <span>{card.price}</span>
                <Button className="wa-btn small">WhatsApp Order</Button>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="container section grid-3">
        ...
        <article className="panel"><h3>Features</h3><ul>{features.map((f) => <li key={f.title}><strong>{f.title}</strong> — {f.text}</li>)}</ul></article>
      </section>
    </main>
  );
}
